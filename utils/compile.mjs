...
import cpConfig from '../cp_config.json' with { type: 'json' };
...
if (!cpConfig.wantCompileToHarmonyOS) {
    // 先获取7zip编译目录
    ...
    let pathOfAmeHarmonyOS = cpConfig.repoRootPathOfAmeHarmonyOS;
    // 准备编译harmonyOS版本
    // 删除旧版本前端和文档
    await fs.promises.rm(`${pathOfAmeHarmonyOS}/app/ame/web_engine/src/main/resources/resfile/resources/app/out`, {recursive: true, force: true});
    await fs.promises.rm(`${pathOfAmeHarmonyOS}/app/ame/web_engine/src/main/resources/resfile/resources/app/document`, {recursive: true, force: true});
    await fs.promises.rm(`${pathOfAmeHarmonyOS}/app/ame/web_engine/src/main/resources/resfile/resources/app/package.json`, {recursive: true, force: true});

    // 拷贝新版本前端和文档
    allShell = `'cd ${path.join(__dirname, "..")} && npm run build && `
        + `cp -r ./out ${pathOfAmeHarmonyOS}/app/ame/web_engine/src/main/resources/resfile/resources/app/out && `
        + `cp -r ./document ${pathOfAmeHarmonyOS}/app/ame/web_engine/src/main/resources/resfile/resources/app/document && `
        + `cp ./package.json ${pathOfAmeHarmonyOS}/app/ame/web_engine/src/main/resources/resfile/resources/app/package.json`;
    console.log(allShell);
}
