# 7-Zip 26.02 —— OpenHarmony / HarmonyOS arm64 交叉编译适配

本目录对 7-Zip 26.02 源码（`libs/third_party/7z2602-src`）做了鸿蒙 arm64
...
```bash
cd libs/third_party/7z2602-src/CPP/7zip/Bundles/Format7zF
...
```text
libs/third_party/7z2602-src/CPP/7zip/Bundles/Format7zF/b/ohos_arm64/7z.so
```

### 方式二：一键脚本（自动拷贝到 out/）

```bash
cd libs/third_party/7z2602-src
...
```text
libs/third_party/7z2602-src/out/ohos-arm64-v8a/7z.so
```
