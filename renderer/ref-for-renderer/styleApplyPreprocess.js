...
function processHTML(htmlStr, rawContent = null) {
    ...
    if (out) {  // 判断是否有元素
        ...
        } else if (out.tagName.toLowerCase() === "blockquote") {  // 自定义提示、警告blockquote
            if (out.children[0]) {
                if (/(\[\!)\S+(\])/.test(out.children[0].innerText)) {  // 符合[!tip]、[!important]、[!note]、[!warning]、[!caution]的通用格式
                    let sigTag = out.children[0].innerText;
                    let sigTagKind = sigTag.replace("[!", "").replace("]", "");
                    let head = document.createElement("p");
                    head.innerHTML = KIND_OF_TIPS_FOR_COLORS_SVGS[sigTag];
                    out.setAttribute("class", `md-alert md-alert-${ sigTagKind } ${ sigTagKind }`);
                    out.setAttribute("style", "margin-bottom: 10px;");
