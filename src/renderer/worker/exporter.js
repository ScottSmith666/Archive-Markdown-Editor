import engine from "../src/components/workspace/engine.js";
import {regExps, returnMediaElement} from "../src/components/workspace/get_media_skeleton.js";

const exportHTML = (originFileContent) => {
    // Markdown-It engine
    ...
    return mdIt.render(originFileContent);
};

const exportPDF = (originFileContent) => {

};

self.addEventListener("message", (e) => {
    if (e.data[0] === 'html') {
        self.postMessage(exportHTML(e.data[1]));
    } else if (e.data[0] === 'pdf') {
        self.postMessage(exportPDF(e.data[1]));
    }
});
